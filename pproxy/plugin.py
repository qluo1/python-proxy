...
class Http_Simple_Plugin(BasePlugin):
    async def init_client_data(self, reader, writer, cipher):
        ...
        writer.write(
            ...
            + datetime.datetime.now().strftime("%a, %d %b %Y %H:%M:%S GMT").encode()
...
class Tls1__2_Ticket_Auth_Plugin(BasePlugin):
    ...
    async def init_client_data(self, reader, writer, cipher):
        ...
        assert (
            hmac.new(key + sessionid, cacheid, hashlib.sha1).digest()[:10]
            == header[28:38]
        )
            ...
            addhmac(
                (
                    ...
                    + packstr(
                        ...
                        + packstr(
                            b"\x03\x03"
                            + addhmac(utc_time.to_bytes(4, "big") + os.urandom(18))
                            + b"\x20"
                            + sessionid
                            + b"\xc0\x2f\x00\x00\x05\xff\x01\x00\x01\x00"
                        )
                    )
                    + (
                        b"\x16\x03\x03"
                        + packstr(
                            b"\x04\x00"
                            + packstr(os.urandom(random.randrange(164) * 2 + 64))
                        )
                        if random.randint(0, 8) < 1
                        else b""
                    )
                    + b"\x14\x03\x03\x00\x01\x01\x16\x03\x03"
                    + packstr(os.urandom(random.choice((32, 40))))
                )[:-10]
            )
        )
    ...
    async def init_server_data(self, reader, writer, cipher, raddr):
        ...
        writer.write(
            ...
            + packstr(
                ...
                + packstr(
                    b"\x03\x03"
                    + addhmac(int(time.time()).to_bytes(4, "big") + os.urandom(18))
                    + b"\x20"
                    + sessionid
                    + b"\x00\x1c\xc0\x2b\xc0\x2f\xcc\xa9\xcc\xa8\xcc\x14\xcc\x13\xc0\x0a\xc0\x14\xc0\x09\xc0\x13\x00\x9c\x00\x35\x00\x2f\x00\x0a\x01\x00"
                    + packstr(
                        b"\xff\x01\x00\x01\x00\x00\x00"
                        + packstr(packstr(b"\x00" + packstr(raddr.encode())))
                        + b"\x00\x17\x00\x00\x00\x23"
                        + packstr(os.urandom((random.randrange(17) + 8) * 16))
                        + b"\x00\x0d\x00\x16\x00\x14\x06\x01\x06\x03\x05\x01\x05\x03\x04\x01\x04\x03\x03\x01\x03\x03\x02\x01\x02\x03\x00\x05\x00\x05\x01\x00\x00\x00\x00\x00\x12\x00\x00\x75\x50\x00\x00\x00\x0b\x00\x02\x01\x00\x00\x0a\x00\x06\x00\x04\x00\x17\x00\x18"
                    )
                )
            )
        )
        writer.write(
            addhmac(b"\x14\x03\x03\x00\x01\x01\x16\x03\x03\x00\x20" + os.urandom(22))
        )
...
PLUGIN = {
    cls.name(): cls for name, cls in globals().items() if name.endswith("_Plugin")
}
