        ...
        if self.KEY_LENGTH > 0 and setup_key:
            ...
            if self.key is None:
                ...
                while len(b"".join(keybuf)) < self.KEY_LENGTH:
                    keybuf.append(
                        hashlib.md5((keybuf[-1] if keybuf else b"") + key).digest()
                    )
                self.key = self.CACHE[b"key" + key] = b"".join(keybuf)[
                    : self.KEY_LENGTH
                ]
        else:
            self.key = key
...
class AES_256_CFB_Cipher(BaseCipher):
    ...
    def setup(self):
        ...
        self.cipher = AES.new(
            self.key, AES.MODE_CFB, iv=self.iv, segment_size=self.SEGMENT_SIZE
        )
...
class BF_CFB_Cipher(BaseCipher):
    ...
    def setup(self):
        ...
        self.cipher = Blowfish.new(
            self.key, Blowfish.MODE_CFB, iv=self.iv, segment_size=64
        )
