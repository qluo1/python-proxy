...
def socks_address(reader, n):
    return (
        socket.inet_ntoa(reader.read(4))
        if n == 1
        else reader.read(reader.read(1)[0]).decode()
        if n == 3
        else socket.inet_ntop(socket.AF_INET6, reader.read(16)),
...
class SSR(BaseProtocol):
    ...
    async def connect(self, reader_remote, writer_remote, rauth, host_name, port, **kw):
        writer_remote.write(
            rauth + b"\x03" + packstr(host_name.encode()) + port.to_bytes(2, "big")
        )
...
class SS(BaseProtocol):
    def correct_header(self, header, auth, **kw):
        return (
            ...
            or not auth
            and header
            and header[0] in (1, 3, 4, 17, 19, 20)
        )
            ...
            while 1:
                if data_len is None:
                    ...
                    assert (
                        _buffer[:10]
                        == hmac.new(
                            cipher.iv + chunk_id.to_bytes(4, "big"), data, hashlib.sha1
                        ).digest()[:10]
                    )
    ...
    async def parse(self, header, reader, auth, authtable, reader_cipher, **kw):
        ...
        host_name, port, data = await socks_address_stream(reader, header[0])
        assert (
            ota or not reader_cipher or not reader_cipher.ota
        ), "SS client must support OTA"
        if ota and reader_cipher:
            checksum = hmac.new(
                reader_cipher.iv + reader_cipher.key, header + data, hashlib.sha1
            ).digest()
    ...
    async def connect(
        ...
        **kw,
        ...
        if writer_cipher_r and writer_cipher_r.ota:
            ...
            checksum = hmac.new(
                writer_cipher_r.iv + writer_cipher_r.key, rdata, hashlib.sha1
            ).digest()
            writer_remote.write(rdata + checksum[:10])
            self.patch_ota_writer(writer_cipher_r, writer_remote)
        else:
            writer_remote.write(
                b"\x03" + packstr(host_name.encode()) + port.to_bytes(2, "big")
            )
    ...
    def udp_client2(self, host_name, port, data):
        try:
            return (
                b"\x01" + socket.inet_aton(host_name) + port.to_bytes(2, "big") + data
            )
        except Exception:
            pass
        ...
        ip = socket.inet_aton(
            (
                await asyncio.get_event_loop().getaddrinfo(
                    host_name, port, family=socket.AF_INET
                )
            )[0][4][0]
        )
        writer_remote.write(
            b"\x04\x01" + port.to_bytes(2, "big") + ip + rauth + b"\x00"
        )
...
class Socks5(BaseProtocol):
    ...
    async def connect(self, reader_remote, writer_remote, rauth, host_name, port, **kw):
        writer_remote.write(
            (
                b"\x05\x01\x02\x01" + b"".join(packstr(i) for i in rauth.split(b":", 1))
                if rauth
                else b"\x05\x01\x00"
            )
            + b"\x05\x01\x00\x03"
        ...
        await reader_remote.read_n(
            6
            if header == 1
            else (18 if header == 4 else (await reader_remote.read_n(1))[0] + 2)
        )
        ...
        try:
            ...
            while True:
                ...
                if b"\r\n" in data and HTTP_LINE.match(
                    ...
                    method, path, ver = HTTP_LINE.match(headers.pop(0)).groups()
                    lines = "\r\n".join(
                        i for i in headers if not i.startswith("Proxy-")
                    )
                    headers = dict(i.split(": ", 1) for i in headers if ": " in i)
                    newpath = (
                        urllib.parse.urlparse(path)
                        ._replace(netloc="", scheme="")
                        .geturl()
                    )
                    data = (
                        f"{method} {newpath} {ver}\r\n{lines}\r\n\r\n".encode() + data
    ...
    async def connect(
        ...
        def write(data, o=writer_remote.write):
            ...
            if pos != -1 or len(buffer) > 4096:
                ...
                data = (
                    f'{method} http://{host_name}{":"+str(port) if port!=80 else ""}{path} {ver}'.encode()
                    + (
                        b"\r\nProxy-Authorization: Basic " + base64.b64encode(rauth)
                        if rauth
                        else b""
    ...
    def query_remote(self, sock):
        try:
            ...
            if "." in sock.getsockname()[0]:
                ...
                assert len(buf) == 28
                return (
                    socket.inet_ntop(socket.AF_INET6, buf[8:24]),
...
class WS(BaseProtocol):
            ...
            while 1:
                if data_len is None:
                    ...
                    if len(_buffer) < required:
                        break
                    data_len = (
                        int.from_bytes(_buffer[2:4], "big")
                        if p == 126
                        else int.from_bytes(_buffer[2:6], "big")
                        if p == 127
                        else p
                    )
                    mask_key = (
                        _buffer[required - 4 : required] if _buffer[1] & 128 else None
                    )
            ...
            if masked:
                ...
                return o(
                    ...
                    + (
                        bytes([data_len | 0x80])
                        if data_len < 126
                        else b"\xfe" + data_len.to_bytes(2, "big")
                        if data_len < 65536
                        else b"\xff" + data_len.to_bytes(4, "big")
                    )
                    + mask_key
                    + data
                )
                ...
                return o(
                    ...
                    + (
                        bytes([data_len])
                        if data_len < 126
                        else b"\x7e" + data_len.to_bytes(2, "big")
                        if data_len < 65536
                        else b"\x7f" + data_len.to_bytes(4, "big")
                    )
                    + data
                )
    ...
    async def parse(self, header, reader, writer, auth, authtable, sock, **kw):
        ...
        rseckey = base64.b64encode(
            hashlib.sha1(seckey + b"amtf").digest()[:16]
        ).decode()
        writer.write(
        ...
        class Handler:
            ...
            async def reader_handler(self):
                try:
                        ...
                        if header in (0x01, 0x03, 0x04, 0x11, 0x13, 0x14):
                            ...
                            if (header & 0x10 == 0) and tcp_handler:
                                ...
                                asyncio.ensure_future(
                                    tcp_handler(
                                        remote_reader, remote_writer, host_name, port
                                    )
                                )
                            ...
                            if header == 0x20 and sid in self.sessions:
                                ...
                                asyncio.ensure_future(
                                    udp_handler(sendto, data, host_name, port, sid)
                                )
                        elif header == 0x40:
                    ...
                    if not isinstance(ex, asyncio.TimeoutError) and not str(
                        ex
                    ...
                    def write(self, data):
                        while len(data) >= 32 * 1024:
                            writer.write(
                                b"\x20"
                                + sid
                                + (32 * 1024).to_bytes(2, "big")
                                + data[: 32 * 1024]
                            )
                            data = data[32 * 1024 :]
                        if data:
                            writer.write(
                                b"\x20" + sid + len(data).to_bytes(2, "big") + data
                            )
            ...
            def get_datagram(self, sid, host_name, port):
                def sendto(data):
                    if data:
                        writer.write(
                            b"\x30" + sid + len(data).to_bytes(2, "big") + data
                        )
