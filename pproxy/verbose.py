...
async def realtime_stat(stats):
    ...
    while True:
        ...
        sys.stdout.write(
            "DIRECT: {5} ({1},{3})   PROXY: {4} ({0},{2})\x1b[0K\r".format(*stat)
        )
        sys.stdout.flush()
        if len(history) >= 10:
            del history[:1]
...
def setup(loop, args):
    def verbose(s):
        if args.v >= 2:
            sys.stdout.write(
                "\x1b[32m" + time.strftime("%Y-%m-%d %H:%M:%S") + "\x1b[m "
            )
    ...
    def modstat(remote_ip, host_name, stats=args.stats):
        host_name_2 = (
            ".".join(
                host_name.split(".")[-3 if host_name.endswith(".com.cn") else -2 :]
            )
            if host_name.split(".")[-1].isalpha()
            else host_name
        )
        tostat = (
            stats[0],
            stats.setdefault(remote_ip, {}).setdefault(host_name_2, [0] * 6),
        )
    ...
    if args.v >= 2:
        ...
        if sys.platform != "win32":
            ...
            loop.run_in_executor(
                None, win_readline, functools.partial(all_stat, args.stats)
            )
