...
async def stream_handler(
    ...
    salgorithm="fa",
    verbose=DUMMY,
    modstat=lambda r, h: lambda i: DUMMY,
    **kwargs,
):
    try:
        if unix:
        ...
        lproto, host_name, port, initbuf = await proto.parse(
            protos,
            reader=reader,
            writer=writer,
        ...
        if host_name == "echo":
            ...
            asyncio.ensure_future(
                lproto.channel(
                    reader_remote, writer, m(2 + roption.direct), m(4 + roption.direct)
                )
            )
            asyncio.ensure_future(
                lchannel(
                    reader, writer_remote, m(roption.direct), roption.connection_change
                )
            )
    except Exception as ex:
        if not isinstance(ex, asyncio.TimeoutError) and not str(ex).startswith(
            "Connection closed"
        ):
...
async def reuse_stream_handler(
    reader,
    writer,
    unix,
    lbind,
    ...
    **kwargs,
    ...
    async def udp_handler(sendto, data, host_name, port, sid):
        try:
            ...
            verbose(
                f"UDP {lproto.name} {remote_text}{roption.logtext(host_name, port)}"
            )
...
async def datagram_handler(
    ...
    **kwargs,
    ...
    try:
        ...
        lproto, host_name, port, data = proto.udp_parse(
        ...
        while not self.closed:
            if self.uri.unix:
                wait = asyncio.open_unix_connection(
                    path=self.uri.bind,
                    ssl=self.uri.sslclient,
                    server_hostname="" if self.uri.sslclient else None,
                )
            else:
                wait = asyncio.open_connection(
                    host=self.uri.host_name,
                    port=self.uri.port,
                    ssl=self.uri.sslclient,
    ...
    def client_run(self):
        ...
        if self.uri.unix:
            return asyncio.start_unix_server(
                handler, path=self.uri.bind, ssl=self.uri.sslserver
            )
        else:
...
class ProxyURI(object):
    ...
    def logtext(self, host, port):
        if self.direct:
            ...
            return (
                f' -> {self.rproto.name+("+ssl" if self.sslclient else "")} {self.bind}'
                + self.relay.logtext(host, port)
            )
    ...
    async def open_udp_connection(self, host, port, data, addr, reply):
        ...
        if addr in self.udpmap:
            ...
            await asyncio.get_event_loop().create_datagram_endpoint(
                lambda: prot, remote_addr=remote_addr
            )
    ...
    def prepare_udp_connection(self, host, port, data):
        if not self.direct:
            ...
            whost, wport = (
                (host, port)
                if self.relay.direct
                else (self.relay.host_name, self.relay.port)
            )
            data = self.rproto.udp_connect(
                rauth=self.auth, host_name=whost, port=wport, data=data
            )
            if self.cipher:
    ...
    async def open_connection(self, host, port, local_addr, lbind):
        if self.reuse or self.ssh:
            if (
                ...
                or self.streams.done()
                and (self.reuse and not self.handler)
            ):
    ...
    def prepare_connection(self, reader_remote, writer_remote, host, port):
        ...
        return self.prepare_ciphers_and_headers(
            reader_remote, writer_remote, host, port, self.handler
        )
    ...
    async def prepare_ciphers_and_headers(
        ...
        if not self.direct:
            ...
            if self.rproto.reuse():
                ...
                reader_remote, writer_remote = await reader_remote.open_connection(
                    whost, wport
                )
            else:
            ...
            return await self.relay.prepare_ciphers_and_headers(
                reader_remote, writer_remote, host, port, handler
            )
    ...
    def start_server(self, args):
        ...
        if self.backward:
            ...
            return asyncio.start_unix_server(
                handler, path=self.bind, ssl=self.sslserver
            )
        else:
            return asyncio.start_server(
                handler,
    ...
    def compile_rule(cls, filename):
        with open(filename) as f:
            return re.compile(
                ...
                + "".join(
                    "|".join(
                        i.strip() for i in f if i.strip() and not i.startswith("#")
                    )
                )
                + ")$"
            ).match
    ...
    def compile(cls, uri, relay=None):
        ...
        if "pack" in protonames and relay and relay != cls.DIRECT:
            raise argparse.ArgumentTypeError(
                "pack protocol cannot relay to other proxy"
            )
        ...
        return ProxyURI(
            ...
            sslserver=sslserver,
            alive=True,
            direct="direct" in protonames,
            tunnel="tunnel" in protonames,
            reuse="pack" in protonames or relay and relay.reuse,
            backward="in" in rawprotos,
            ssh="ssh" in rawprotos,
            relay=relay,
        )


ProxyURI.DIRECT = ProxyURI(
    direct=True,
    tunnel=False,
    reuse=False,
    relay=None,
...
async def test_url(url, rserver):
    ...
    for roption in rserver:
        ...
        try:
            reader, writer = await roption.prepare_connection(
                reader, writer, host_name, port
            )
        except Exception:
            writer.close()
            raise Exception("Unknown remote protocol")
...
def main():
    ...
    parser.add_argument(
        "-b", dest="block", type=ProxyURI.compile_rule, help="block regex rules"
    )
    parser.add_argument(
        "-a",
        dest="alived",
    ...
    parser.add_argument(
        ...
        dest="ruport",
        action="store_true",
        help="set SO_REUSEPORT (Linux only)",
    )
    parser.add_argument(
        "--daemon",
        dest="daemon",
    ...
    if args.pac:
        pactext = "function FindProxyForURL(u,h){" + (
            f'var b=/^(:?{args.block.__self__.pattern})$/i;if(b.test(h))return "";'
            if args.block
            else ""
        )
        for i, option in enumerate(args.rserver):
            pactext += (
                f"var m{i}=/^(:?{option.match.__self__.pattern})$/i;if(m{i}.test(h))"
                if option.match
                else ""
            ) + 'return "PROXY %(host)s";'
        ...
        args.httpget[
            args.pac + "/all"
        ] = 'function FindProxyForURL(u,h){return "PROXY %(host)s";}'
        args.httpget[
            args.pac + "/none"
    ...
    for option in args.listen:
        print(
            ...
            option.bind,
            "by",
            ",".join(i.name for i in option.protos)
            + ("(SSL)" if option.sslclient else ""),
            "({}{})".format(
                option.cipher.name,
                " " + ",".join(i.name() for i in option.cipher.plugins)
                if option.cipher and option.cipher.plugins
                else "",
            )
            if option.cipher
            else "",
    ...
    for option in args.ulisten:
        ...
        try:
            server, protocol = loop.run_until_complete(
                option.start_udp_server(vars(args))
            )
            servers.append(server)
        except Exception as ex:
            print("Start server failed.\n\t==>", ex)
    ...
    if servers:
        ...
        if args.alived > 0 and args.rserver:
            asyncio.ensure_future(
                check_server_alive(
                    args.alived, args.rserver, args.verbose if args.v else DUMMY
                )
            )
        try:
            loop.run_forever()
