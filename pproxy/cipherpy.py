    ...
    def core(self):
        data = list(
            struct.unpack(
                ...
                + self.iv.rjust(12, b"\x00"),
            )
        )
        while 1:
            yield from struct.pack(
                "<16I",
                *(a + b & 0xFFFFFFFF for a, b in zip(ChaCha20_round(data[:]), data)),
            )
            data[12:14] = (
                (0, data[13] + 1)
                if data[12] == 0xFFFFFFFF
                else (data[12] + 1, data[13])
            )
...
class XChaCha20_Cipher(ChaCha20_Cipher):
    ...
    def core(self):
        H = ChaCha20_round(
            list(struct.unpack("<16I", b"expand 32-byte k" + self.key + self.iv[:16]))
        )
...
def poly1305(cipher_encrypt, nonce, ciphertext):
    ...
    mac_data = (
        ciphertext
        + bytes((-len(ciphertext)) % 16 + 8)
        + len(ciphertext).to_bytes(8, "little")
    )
    acc, r, s = (
        0,
        int.from_bytes(otk[:16], "little") & 0x0FFFFFFC0FFFFFFC0FFFFFFC0FFFFFFF,
        int.from_bytes(otk[16:], "little"),
    )
    for i in range(0, len(mac_data), 16):
        acc = (r * (acc + int.from_bytes(mac_data[i : i + 16] + b"\x01", "little"))) % (
            (1 << 130) - 5
        )
...
class ChaCha20_IETF_POLY1305_Cipher(AEADCipher):
    ...
    def setup(self):
        self.cipher_encrypt = (
            lambda nonce, s, counter=0: ChaCha20_IETF_Cipher(
...
class XChaCha20_IETF_POLY1305_Cipher(ChaCha20_IETF_POLY1305_Cipher):
    ...
    def setup(self):
        self.cipher_encrypt = (
            lambda nonce, s, counter=0: XChaCha20_IETF_Cipher(
                self.key, setup_key=False, counter=counter
            )
            .setup_iv(nonce)
            .encrypt(s)
    ...
    def core(self):
        ...
        while 1:
            ...
            for a, b, c, d in ORDERS_SALSA20:
                ...
                H[b] ^= ROL(H[c] + H[d], 18)
            yield from struct.pack(
                "<16I", *(a + b & 0xFFFFFFFF for a, b in zip(H, data))
            )
            data[8:10] = (
                (0, data[9] + 1) if data[8] == 0xFFFFFFFF else (data[8] + 1, data[9])
            )
...
class CFBCipher(StreamCipher):
    def setup(self):
        ...
        self.stream = (
            self.core_bit(segment_bit) if self.bit_mode else self.core(segment_bit // 8)
        )
        self.last = None
    ...
    def core_bit(self, segment_bit):
        ...
        while 1:
            ...
            for i in range(segment_bit):
                next_iv |= (yield data[i // 8] >> (7 - i % 8) & 1) << (
                    segment_bit - 1 - i
                )
...
class GCMCipher(AEADCipher):
    ...
    def process(self, s, tag=None):
        ...
        ret = bytes(
            ...
            for j, o in enumerate(self.cipher.encrypt(z | (i + 2) & ((1 << 32) - 1)))
            if i * 16 + j < len(s)
        )
        if tag is None:
...
class AES(RAW):
    g1 = base64.b64decode(
        b""
    )
    g2 = [a << 1 & 0xFF ^ 0x1B if a & 0x80 else a << 1 for a in g1]
    g3 = [a ^ (a << 1 & 0xFF ^ 0x1B if a & 0x80 else a << 1) for a in g1]
    Rcon = base64.b64decode(
        b"jQECBAgQIECAGzZs2KtNmi9evGPGlzVq1LN9+u/FkTly5NO9YcKfJUqUM2bMgx06dOjL"
    )
    shifts = tuple(
        (
            j,
            j & 3 | ((j >> 2) + (j & 3)) * 4 & 12,
            j + 3 & 3 | ((j >> 2) + (j + 3 & 3)) * 4 & 12,
            j + 2 & 3 | ((j >> 2) + (j + 2 & 3)) * 4 & 12,
            j + 1 & 3 | ((j >> 2) + (j + 1 & 3)) * 4 & 12,
        )
        for j in range(16)
    )
    ...
    def encrypt(self, data):
        ...
        for key in self.ekey[1:-1]:
            s = [
                self.g2[s[a]] ^ self.g1[s[b]] ^ self.g1[s[c]] ^ self.g3[s[d]] ^ key[j]
                for j, a, b, c, d in self.shifts
            ]
        return bytes(
            [self.g1[s[self.shifts[j][1]]] ^ self.ekey[-1][j] for j in range(16)]
        )
...
for method in (CFBCipher, CFB8Cipher, CFB1Cipher, CTRCipher, OFBCipher, GCMCipher):
    for key in (32, 24, 16):
        ...
        globals()[name] = type(
            ...
            (method,),
            dict(
                KEY_LENGTH=key, IV_LENGTH=key if method is GCMCipher else 16, CIPHER=AES
            ),
        )
...
class Blowfish(RAW):
    ...
    def __init__(self, key):
        if not self.P:
            pi = self.hex_pi()
            self.__class__.P = [
                int("".join(next(pi) for j in range(8)), 16) for i in range(18 + 1024)
            ]
        self.p = [
            a ^ b
            for a, b in zip(
                self.P[:18], struct.unpack(">18I", (key * (72 // len(key) + 1))[:72])
            )
        ] + self.P[18:]
        buf = b"\x00" * 8
    ...
    def encrypt(self, s):
        ...
        for i in self.p[1:17]:
            sl, sr = (
                ...
                ^ i
                ^ (
                    self.p[18 + (sl >> 24)] + self.p[274 + (sl >> 16 & 0xFF)]
                    ^ self.p[530 + (sl >> 8 & 0xFF)]
                )
                + self.p[786 + (sl & 0xFF)]
                & 0xFFFFFFFF,
                sl,
...
class Camellia(RAW):
    ...
    def R(self, s, t):
        ...
        return (
            ...
            ^ t << 16 & 0xFF << 56
            ^ t << 56
            ^ (t >> 32 << 48 ^ t >> 16 ^ t << 24) & 0xFFFFFF << 32
            ^ s
        ) << 64 & (1 << 128) - 1 | s >> 64

    def __init__(self, key):
        q, R = (
            [int.from_bytes(key[:16], "big"), int.from_bytes(key[16:], "big"), 0, 0],
            self.R,
        )
        q[1] = q[1] << 64 | q[1] ^ ((1 << 64) - 1) if len(key) == 24 else q[1]
        q[2] = R(
            R(
                R(R(q[0] ^ q[1], 0xA09E667F3BCC908B), 0xB67AE8584CAA73B2) ^ q[0],
                0xC6EF372FE94F82BE,
            ),
            0x54FF53A5F1D36F1C,
        )
        ...
        self.e = [
            e[i + i // 7] << 64 | e[i + i // 7 + 1] if i % 7 == 0 else e[i + i // 7 + 1]
            for i in range(nr)
        ]
    ...
    def encrypt(self, s):
        ...
        for idx, k in enumerate(self.e[1:-1]):
            s = (
                s
                ^ ((s & k) >> 95 & 0xFFFFFFFE | (s & k) >> 127) << 64
                ^ ((s & k) << 1 & ~1 << 96 ^ (s & k) >> 31 ^ s << 32 | k << 32)
                & 0xFFFFFFFF << 96
                ^ ((s | k) & 0xFFFFFFFF) << 32
                ^ ((s | k) << 1 ^ s >> 31) & k >> 31 & 0xFFFFFFFE
                ^ ((s | k) >> 31 ^ s >> 63) & k >> 63 & 1
                if (idx + 1) % 7 == 0
                else self.R(s, k)
            )
...
class IDEA(RAW):
    def __init__(self, key):
        ...
        for i in range(8, 52):
            e.append(
                (e[i - 8 & 0xF8 | i + 1 & 0x7] & 0x7F) << 9
                | e[i - 8 & 0xF8 | i + 2 & 0x7] >> 7
            )
    ...
    def encrypt(self, s):
        ...
        e = self.e[-1]
        return struct.pack(
            ">4H", M(s0, e[0]), s2 + e[1] & 0xFFFF, s1 + e[2] & 0xFFFF, M(s3, e[3])
        )
...
class SEED(RAW):
    ...
    def __init__(self, key):
        ...
        for i, kc in enumerate(
            ...
            self.e.append(
                (self.G((key0 >> 32) + (key1 >> 32) - kc), self.G(key0 - key1 + kc))
            )
            key0, key1 = (
                (key0, (key1 << 8 | key1 >> 56) & (1 << 64) - 1)
                if i & 1
                else ((key0 << 56 | key0 >> 8) & (1 << 64) - 1, key1)
            )
